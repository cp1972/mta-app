...
Overview of major changes:
  ...
  - Estimation of the best number of topics: we have now five metrics (before MTA-1.0, only two) to help in the decision regarding the best number of topics, including silhouette (for Kmeans++ and Word2Vec), [https://scikit-learn.org/stable/modules/clustering.html#davies-bouldin-index](Davis Bouldin), [https://scikit-learn.org/stable/modules/mixture.html](BIC and Log-Likelihood) (one plot to summarize all results easing the comparison)
  ...
  - TSNE for word embeddings: we now use Word2Vec with TSNE and the [https://scikit-learn.org/stable/modules/clustering.html#affinity-propagation|affinity](propagation algorithm) to plot semantic similarities for chosen words of the corpus, and to inspect semantic clusters based on these chosen words
  - PyLDAVIS: it has been dropped because of the [https://github.com/cpsievert/LDAvis/pull/41](bug) related to the _.prepare method of pyLDAvis in scikit-learn regarding the red bars -- they do not give the estimated frequencies of words qua topic (The implementation is there in MTA for both NMF and LDA, and as soon as this bug has been corrected, we will reintroduce pyLDAvis as another way to look at the results of the topic model analysis)
